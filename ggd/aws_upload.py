...
# Returns:
# 1) a string representing the updated build number meta.yaml file
def update_meta_yaml(tarball_info_object):
	...
	for line in tarball_info_object:
		if "about:" in line:
			aboutkey = True	
		if "summary:" in line and aboutkey == True:
			meta_str += "  tags: uploaded_to_aws\n" ## Add tags key to about field
			aboutkey = False
			meta_str += line
			continue
		if "build:" in line:
			buildkey = True
		if "number:" in line and buildkey == True:
			build_num = re.search('\d+',line).group()  
			print("\tNew build number: %d" %(int(build_num) + 1))
			new_build_str = re.sub(build_num, str(int(build_num) + 1), line)
			buildkey = False
			meta_str += new_build_str	
		else:
			meta_str += line
...
with tarfile.open(args.tarfile, "r:bz2") as tarball_file:
	...
	write_file(new_file_path,"meta.yaml",meta_yaml_str)
