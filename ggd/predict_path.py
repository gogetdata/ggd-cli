...
def add_predict_path(p):
    c = p.add_parser('predict-path', help="Predict the install file path of a data package that hasn't been installed yet. (Use for workflows, such as Snakemake)", description="Get a predicted install file path for a data package before it is installed. (Use for worklows, such as Snakemake)")
    ...
    c.add_argument("--prefix", default=None, help="(Optional) The name or the full directory path to an conda environment. The predicted path will be based on this conda environment. When installing, the data package should also be installed in this environment. (Only needed if not predicting for a path in the current conda enviroment)") 
    c2 = c.add_argument_group("required arguments")
    c2.add_argument("-pn", "--package-name", required=True, help="(Required) The name of the data package to predict a file path for") 
    c2.add_argument("-fn", "--file-name", required=True, help="(Required) The name of the file to predict that path for. It is best if you give the full and correct name of the file to predict the path for. If not, ggd will try to identify the right file, but won't guarantee that it is the rigth file")
...
def get_ggd_metadata(ggd_channel):
    ...
    if check_for_internet_connection(3): 
        ...
        sys.exit("\n\t-> A internet connection is required to use this function. Please try again when you have secured an internet connection\n")
...
def predict_path(parser, args):
    ...
    if args.package_name not in metadata_dict["packages"]:
        sys.exit("\n\t-> The {pn} data package is not one of the packages in the ggd-{c} channel\n".format(pn=args.package_name, c=args.channel))

    ## Check there is a "final-files" in the metadata for the package
    if "final-files" not in metadata_dict["packages"][args.package_name]["tags"] or len(metadata_dict["packages"][args.package_name]["tags"].get("final-files", [])) == 0:
        sys.exit("\n\t-> The {p} data package does not have the final data files listed. This packages needs to be updated. To update, contact the GoGetData team at https://github.com/gogetdata/ggd-recipes\n".format(p=args.package_name))
        
    ## Check that the file is one of the final-files listed in the metadata
    if args.file_name not in metadata_dict["packages"][args.package_name]["tags"]["final-files"]:
        matching_files = [x for x in metadata_dict["packages"][args.package_name]["tags"]["final-files"] if re.search(args.file_name,x)]
        if len(matching_files) > 0:
            ## Chose the first file that matched
            file_name = matching_files[0]
        else:
            sys.exit("\n\t-> The {f} file is not one of the files listed for this package. The files installed by this package are: \n\t\t{fo}".format(f=args.file_name, fo="\n\t\t".join(metadata_dict["packages"][args.package_name]["tags"]["final-files"])))
    else:
    ...
    ## Print the path
    path = os.path.join(CONDA_ROOT, "share", "ggd", species, build, args.package_name, version, file_name)
    print(path)
