...
def add_install(p):
    ...
    c.add_argument("--file",default=[], action="append",help="A file with a list of ggd data packages to install. One package per line. Can use more than one (e.g. ggd install --file <file_1> --file <file_2> )")
    c.add_argument("name", nargs="*", help="The data package name to install. Can use more than once (e.g. ggd install <pkg 1> <pkg 2> <pkg 3> ). (NOTE: No need to designate version as it is implicated in the package name)")
...
def check_ggd_recipe(ggd_recipe,ggd_channel):
    ...
    if ggd_recipe in package_list:
        print("\n:ggd:install: %s exists in the ggd-%s channel" %(ggd_recipe,ggd_channel))
        return(jdict)
    else:
        print("\n:ggd:install: '%s' was not found in ggd-%s" %(ggd_recipe, ggd_channel))
        print(":ggd:install:\t You can search for recipes using the ggd search tool: \n\t\t'ggd search %s'\n" %ggd_recipe)
        return(None)
...
def check_if_installed(ggd_recipe,ggd_jdict,prefix=None):
    ...
    if recipe_exists:
        ...
        print("\n:ggd:install: %s version %s is not installed on your system" %(ggd_recipe, version))
        return(False)
...
def check_conda_installation(ggd_recipe,prefix=None):
    ...
    if ggd_recipe not in conda_package_list.keys(): 
        ...
        print("\n:ggd:install: %s has been installed by conda on your system and must be uninstalled to proceed." %ggd_recipe)
        print(":ggd:install:\t To reinstall run:\n\t\t $ ggd uninstall %s \n\t\t $ ggd install %s" %(ggd_recipe,ggd_recipe))
        if prefix != conda_root():
            print(":ggd:install:\t NOTE: You must activate the conda prefix {p} before running the uninstall command".format(p = target_prefix)) 
        sys.exit()
    ...
    if "tags" in ggd_jdict["packages"][ggd_recipe]:
        if "cached" in ggd_jdict["packages"][ggd_recipe]["tags"]:
            if "uploaded_to_aws" in ggd_jdict["packages"][ggd_recipe]["tags"]["cached"]:
                print("\n:ggd:install: The %s package is uploaded to an aws S3 bucket. To reduce processing time the package will be downloaded from an aws S3 bucket" %ggd_recipe)
                return(True)
    return(False)
...
def install_from_cached(ggd_recipes, ggd_channel,ggd_jdict,debug=False,prefix=None):
    ...
    """
    from .utils import bypass_satsolver_on_install, update_installed_pkg_metadata, ChecksumError
    ...
    try:
        ...
        check_for_installation(ggd_recipes,ggd_jdict, target_prefix) ## .uninstall method to remove extra ggd files
        print("\n:ggd:install: %s was not installed. Please correct the errors and try again." %", ".join(ggd_recipes))
        sys.exit(1) 
    ...
    target_prefix = prefix if prefix != None else conda_root() 
    update_installed_pkg_metadata(prefix=target_prefix,remove_old=False,add_packages=ggd_recipes)
...
def conda_install(ggd_recipes, ggd_channel,ggd_jdict, debug=False, prefix=None):
    ...
    True if successful install
    """
    from .utils import get_required_conda_version, update_installed_pkg_metadata, ChecksumError
...
def get_file_locations(ggd_recipes,ggd_jdict,prefix=None):
            ...
            if len(files) == 1: ## A single file will have a env var 
               file_var = "$ggd_%s_file" %ggd_recipe.replace("-","_")
                ...
                if [True for x in files if ".tbi" in x or ".bai" in x or ".crai" in x or ".fai" in x or ".gzi" in x]:
                    file_var = "$ggd_%s_file" %ggd_recipe.replace("-","_").replace(".","_")
    ...
    print(dash,"\n")
    print(":ggd:install: To activate environment variables run `source activate base` in the environmnet the packages were installed in\n")
    if prefix != None:
        print(":ggd:install: NOTE: These environment variables are specific to the %s conda environment and can only be accessed from within that environmnet" %prefix)
    print("======================================================================================================================")
    print("\n\n")
...
def install_checksum(pkg_names,ggd_jdict,prefix=conda_root()):
    ...
    """
    import tarfile
    from .utils import get_conda_package_list, get_checksum_dict_from_tar, data_file_checksum, ChecksumError
    ...
    for pkg_name in pkg_names:
        ...
        with tarfile.open(tarfile_path,mode="r|bz2") as tar:
            if "info/recipe/checksums_file.txt" not in tar.getnames():
                print("\n:ggd:install: WARNING: Checksum file not available for the {} data package. Data file content validation will be skipped".format(pkg_name)) 
                continue
        ...
        install_path = os.path.join(prefix,"share","ggd",species,build,pkg_name,version)

        try:
            assert os.path.exists(install_path)
        except AssertionError as e:
            print("\n:ggd:install: !!ERROR!! There was an error durring the installation of {} and the files weren't installed correctly".format(pkg_name))
...
def install(parser, args):
    ...
    if args.file:
        for file in args.file:
            assert os.path.exists(file), ("\n\n:ggd:install: !!ERROR!! The {f} file provided does not exists".format(f=file))
            with open(file, "r") as f:
                pkg_list.extend([x.strip() for x in f if len(x.strip().split(" ")) == 1])
            
    ## Check each package
    install_list = []
    for pkg in sorted(pkg_list):
        print("\n\n:ggd:install: Looking for %s in the 'ggd-%s' channel" %(pkg,args.channel))
    ...
    if cached:
        ...
        install_from_cached(cached, args.channel, ggd_jsonDict, debug=args.debug,prefix=conda_prefix)           
                
    if non_cached:
        print("\n\n:ggd:install:   Attempting to install the following non-cached package(s):\n\t{}\n".format("\n\t".join(non_cached)))
        conda_install(non_cached, args.channel, ggd_jsonDict,debug=args.debug,prefix=conda_prefix)
