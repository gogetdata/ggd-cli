...
def add_search(p):
    import argparse
    c = p.add_parser("search", help="Search for a ggd data package", description="Search for available ggd data packages. Results are filtered by match score from high to low. (Only 5 results will be reported unless the -dn flag is changed)")
    c.add_argument("search_term", nargs="+", help="**Required** The term(s) to search for. Multiple terms can be used. Example: 'ggd search reference genome'")
    c.add_argument("-g", "--genome-build",default=[],action="append", choices=[str(x) for x in GENOME_BUILDS], help="(Optional) Filter results by the genome build of the desired recipe")
    c.add_argument("-s", "--species", default=[], action="append", help="(Optional) Filter results by the species for the desired recipe", choices=[str(x) for x in SPECIES_LIST])
    c.add_argument("-dn","--display-number", default=5, help="(Optional) The number of search results to display. (Default = 5)")
    c.add_argument("-m", "--match-score", default="75", help="(Optional) A score between 0 and 100 to use to filter the results by. (Default = 75). The lower the number the more results will be output")
...
def search_packages(json_dict,search_terms,score_cutoff=50):
    ...
    for term in search_terms:
        for pkg in json_dict["packages"].keys():
            ...
            additional_scores = process.extract(term, json_dict["packages"][pkg]["keywords"])
    ...
    temp_list = sorted([[pkg,int(max_score)] for pkg,max_score in pkg_score.items() if max_score >= score_cutoff], key = lambda x: x[1], reverse = True) 
...
def filter_by_identifiers(iden_keys,json_dict,filter_terms):
            ...
            for i,iden_key in enumerate(iden_keys):
                if iden_key in json_dict["packages"][key]["identifiers"]:
                    if len(filter_terms[i]) == 0:
                        continue
                    if filter_terms[i] in json_dict["packages"][key]["identifiers"][iden_key]:
    ...
    if len(new_json_dict["packages"].keys()) == key_count:
        ...
        print("\n:ggd:search: WARNING: Unable to filter packages using: '%s'" %", ".join(filter_terms))
...
def print_summary(search_terms,json_dict,match_list,installed_pkgs,installed_paths):
    ...
    if len(match_list) < 1:
        print("\n:ggd:search: No results for %s. Update your search term(s) and try again" %", ".join(search_terms))
        sys.exit()
    print("\n",dash)
    for pkg in match_list:
        ...
        if pkg in json_dict["packages"]:
            ...
            if "summary" in json_dict["packages"][pkg] and json_dict["packages"][pkg]["summary"]:
                results.append("\t{} {}".format(("\033[1m" + "Summary:" + "\033[0m"), json_dict["packages"][pkg]["summary"]))
            if "identifiers" in json_dict["packages"][pkg] and json_dict["packages"][pkg]["identifiers"]:
                results.append("\t{} {}".format(("\033[1m" + "Species:" + "\033[0m"), json_dict["packages"][pkg]["identifiers"]["species"]))
                results.append("\t{} {}".format(("\033[1m" + "Genome Build:" + "\033[0m"), json_dict["packages"][pkg]["identifiers"]["genome-build"]))
            if "keywords" in json_dict["packages"][pkg] and json_dict["packages"][pkg]["keywords"]: 
                results.append("\t{} {}".format(("\033[1m" + "Keywords:" + "\033[0m"), ", ".join(json_dict["packages"][pkg]["keywords"])))
            if "tags" in json_dict["packages"][pkg] and json_dict["packages"][pkg]["tags"]:
                if "cache" in json_dict["packages"][pkg]["tags"]:
                    results.append("\t{} {}".format(("\033[1m" + "Cached:" + "\033[0m"), json_dict["packages"][pkg]["tags"]["cached"]))
                if "data-provider" in json_dict["packages"][pkg]["tags"]:
                    results.append("\t{} {}".format(("\033[1m" + "Data Provider" + "\033[0m"), json_dict["packages"][pkg]["tags"]["data-provider"]))
                if "data-version" in json_dict["packages"][pkg]["tags"]:
                    results.append("\t{} {}".format(("\033[1m" + "Data Version:" + "\033[0m"), json_dict["packages"][pkg]["tags"]["data-version"]))
                if "file-type" in json_dict["packages"][pkg]["tags"]:
                    results.append("\t{} {}".format(("\033[1m" + "File type(s):" + "\033[0m"), ", ".join(json_dict["packages"][pkg]["tags"]["file-type"])))
                if "genomic-coordinate-base" in json_dict["packages"][pkg]["tags"]:  
                    results.append("\t{} {}".format(("\033[1m" + "Data file coordinate base:" + "\033[0m"),json_dict["packages"][pkg]["tags"]["genomic-coordinate-base"])) 
                if "final-files" in json_dict["packages"][pkg]["tags"]:
                    results.append("\t{} {}".format(("\033[1m" + "Included Data Files:" + "\033[0m"), "\n\t\t"+"\n\t\t".join(json_dict["packages"][pkg]["tags"]["final-files"])))

            if pkg in installed_pkgs: ## IF installed
                results.append("\n\tThis pacakge is already installed on your system.\n\t  You can find the installed data files here:  %s" %installed_paths[pkg])
            else:
                results.append("\n\tTo install run:\n\t\tggd install %s" %pkg)
        print("\n\n".join(results))
        print("\n",dash)
...
def search(parser, args):
    ...
    if args.genome_build or args.species:
        j_dict = filter_by_identifiers(["species"]*len(args.species)+["genome-build"]*len(args.genome_build),j_dict,args.species+args.genome_build)

    ## Search pkg names and keywords 
    match_results = search_packages(j_dict,filtered_search_terms,int(args.match_score))
